...
//#define DEBUG_DHT
#ifdef DEBUG_DHT
#define debug(fmt, ...) ESP_LOGI(TAG, "%s" fmt "\n", "dht: ", ## __VA_ARGS__);
#else
 ...
 */
static bool dht_await_pin_state(uint8_t pin, uint32_t timeout, int expected_pin_state, uint32_t *duration) {
...
static inline bool dht_fetch_data(uint8_t pin, bool bits[DHT_DATA_BITS]) {
    ...
    if (!dht_await_pin_state(pin, 40, 0, NULL)) {
      debug("Initialization error, problem in phase 'B'\n");
        return false;
    }

    // Step through Phase 'C', 88us
    if (!dht_await_pin_state(pin, 88, 1, NULL)) {
        debug("Initialization error, problem in phase 'C'\n");
        return false;
    }

    // Step through Phase 'D', 88us
    if (!dht_await_pin_state(pin, 88, 0, NULL)) {
    ...
    for (int i = 0; i < DHT_DATA_BITS; i++) {
        if (!dht_await_pin_state(pin, 65, 1, &low_duration)) {
            debug("LOW bit timeout\n");
            return false;
        }
        if (!dht_await_pin_state(pin, 75, 0, &high_duration)) {
